...
function recursionListToFormat(_resList) {
...
*/
function _addCoordinateRange(resList) {
    for (let item of resList) {
...
function _checkPosIsExist({ x, y }, resList, resKey) {
...
/**
   ...
   */
function _hideRectById(areaID, _layerObj) {
...
/**
    ...
    * @param _layerObj --- 当前 layer 图层
*/
function _hideRectAndLineById(areaID, _layerObj) {
    for (let item of _layerObj.children) {
        // 隐藏背景矩形
        if (item.className == "Rect" && item.rectType == 'bgcRect') {
            if (item.rectId == areaID) {
...
function cleanAllRectOrLine(_layerObj){
    for (let item of _layerObj.children) {
        if (item.className == "Rect" && item.rectType == 'bgcRect') {
            item.hide();
        }
...
/**
    ...
    * @param _layerObj --- 当前 layer 图层
   */
function _chooseRectById(areaID, _layerObj) {
    for (let item of _layerObj.children) {
        if (item.className == "Rect" && item.rectType == 'bgcRect') {
            if (item.rectId == areaID) {
                item.show();
